...
public:
    ...
    int pos;
    int Ex_a[16][124];//存兑换区地址
    int Ex_i[16]; //用于Ex_a数组
    int free;//空闲块数
      ...
      if(str.length()>64){
        ...
        if(Mb_num<=free) {
          ...
          for(int j=0; j<Mb_num-free; j++) {
            ...
            Ex_a[thread_id][Ex_i[thread_id]]=A_Disk.swap(Mem[z].message);//换到置换区
            Ex_i[thread_id]++;
            q.pop();
            Mem[z].set();
    ...
    int Delete(int thread_id) { //释放内存
      ...
      for(int i=0; i<Ex_i[thread_id]; i++) { //调用磁盘管理的函数删除兑换区对应的文件
